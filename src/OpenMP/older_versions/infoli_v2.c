...
int main(int argc, char *argv[]){
	...
	/* initial passing of the connections file
 	 ...
 	 */
		...
		for (i=0; i<IO_NETWORK_SIZE; i++) {
			cellParamsPtr[i].neighVdend = (mod_prec*) _mm_malloc(cellParamsPtr[i].total_amount_of_neighbours*sizeof(mod_prec), 64);
			...
			cellParamsPtr[i].total_amount_of_neighbours = 0;
/*
			posix_memalign(cellParamsPtr[i].neighVdend, 64, cellParamsPtr[i].total_amount_of_neighbours*sizeof(mod_prec));
                        posix_memalign(cellParamsPtr[i].neighConductances, 64, cellParamsPtr[i].total_amount_of_neighbours*sizeof(mod_prec));
                        posix_memalign(cellParamsPtr[i].neighId, 64, cellParamsPtr[i].total_amount_of_neighbours*sizeof(int));
                        cellParamsPtr[i].total_amount_of_neighbours = 0;
*/		}
...
//				CompAxon(&cellParamsPtr[target_cell]);
		...
		#pragma omp parallel for shared (cellParamsPtr, cellPtr, pOutFile) private(iApp, target_cell, tempbuf, i, requested_neighbour) firstprivate(simulation_array_ID)
			...
			#pragma ivdep
				...
				__assume_aligned(cellParamsPtr[target_cell].neighVdend, 64);
				for (i=0; i<cellParamsPtr[target_cell].total_amount_of_neighbours; i++){
                                        requested_neighbour = cellParamsPtr[target_cell].neighId[i];
...
//				Neighbours Current Calcs
			...
			#pragma ivdep
				...
				for (i=0; i<cellParamsPtr[target_cell].total_amount_of_neighbours; i++) {
					V = cellParamsPtr[target_cell].prevCellState->dend.V_dend - cellParamsPtr[target_cell].neighVdend[i];
...
//				Dendritic Voltage And Current Cacls
				...
				dVd_dt = (-(I_CaH + I_sd+ I_ld + I_K_Ca + I_c + I_h) + cellParamsPtr[target_cell].iAppIn) / C_M;
...
//		#pragma omp parallel for simd shared (cellParamsPtr, cellPtr, pOutFile) private(iApp, target_cell, tempbuf, i, requested_neighbour) firstprivate(simulation_array_ID)
//			for (target_cell=0;target_cell<cellCount;target_cell++) {
//				q_inf = 1 /(1 + expf((cellParamsPtr[target_cell].prevCellState->dend.V_dend + 80) / 4));
//                                tau_q = 1 /(expf(-0.086f * cellParamsPtr[target_cell].prevCellState->dend.V_dend - 14.6f) + expf(0.070f * cellParamsPtr[target_cell].prevCellState->dend.V_dend - 1.87f));
//                                dq_dt = (q_inf - cellParamsPtr[target_cell].prevCellState->dend.Hcurrent_q) / tau_q;
