...
int main(int argc, char *argv[]){
	...
	//debugging purposes
/*	printf("after make core communication list\n");
	temp = sending_list_head;
	while (temp != NULL){
		printf("inside the while loop\n");
		for (i=0; i< temp->total_cells_to_send; i++)
			printf("cell id %d ", temp->cells_to_send[i]);
		printf("\n");
...
//			sprintf(tempbuf, "[%d][%d] ", cellPtr[0][i].cell_x, cellPtr[0][i].cell_y);
	...
	if(inputFromFile){
		...
		for(simStep=0;simStep<total_simulation_steps;simStep++) {
			...
			for (target_cell=0;target_cell<cellCount;target_cell++) {
				...
				for (i=0; i<cellParamsPtr[target_cell].total_amount_of_neighbours; i++) {//we will run through the neighbours of the cell
					found = 0;
					requested_neighbour = cellParamsPtr[target_cell].neighId[i];      //global id of the next neighbour this cell needs
					core = requested_neighbour/cellCount; //the core that this neighbor belongs to
					while (core != r_temp->target_core){
						r_temp = r_temp->next;
						last_index = 0;
					}	//i locate the core's node in the list

					if (core == core_id){	//this neighbour is in my core
						my_requested_cell = requested_neighbour%cellCount;
						cellParamsPtr[target_cell].neighVdend[i] = cellPtr[simulation_array_ID][my_requested_cell].dend.V_dend;
					}
					else {		//the neighbour belongs to another core
						if (simStep == 0){		//we need to find the position of the neighbour in the buffer once and then track it
							while ((last_index <= r_temp->total_cells_to_receive)&&(found != 1)){
								...
								if (requested_neighbour == translated_cell_id){			//if the ids match, store it and track it
									cellParamsPtr[target_cell].neighVdend[i] = r_temp->voltages_to_receive[last_index];
									found = 1;
									cellParamsPtr[target_cell].neighSlots[i] = last_index;	//neighSlots is the tracker structure
								}
								last_index++;
							}
						}
						else		//since we have tracked the exact location of the neighbour we need, we swiftly store it
							cellParamsPtr[target_cell].neighVdend[i] = r_temp->voltages_to_receive[cellParamsPtr[target_cell].neighSlots[i]];
					}
				}
	...
	if (PRINTSTATE) {
		...
		sprintf(resultFileName, "lastStateDump%d.txt", core_id);
...
*/
	...
	int** history = (int**) calloc(cores, sizeof(int*));
	mod_prec cond_value;
	...
	//we read the matrix once and store the number of neighbours for each cell of the core
	for (line_counter=0;line_counter<IO_NETWORK_SIZE;line_counter++){
		...
		for (i=0; i<IO_NETWORK_SIZE; i++) {
			...
			if (cond_value==0)						//this connection is considered not existing if conductance = 0
				...
				if ((i>=my_cell_lower_bound)&&(i<my_cell_upper_bound)) {
	...
	//we allocate the necessary memory for storing the information required for each cell
	//the problem here is that the change from single to double precision and vice versa needs to be done manually
	for (i=0;i<cellCount;i++){
		core_cells[i].neighVdend = (mod_prec*) calloc(my_neighbour_count[i], sizeof(mod_prec));
		core_cells[i].neighConductances = (mod_prec*) calloc(my_neighbour_count[i], sizeof(mod_prec));
		core_cells[i].neighId = (int*) calloc(my_neighbour_count[i], sizeof(int));
		core_cells[i].neighSlots = (int*) calloc(my_neighbour_count[i], sizeof(int));
	}
	
	//Put zero values to the my_neighbour_count array in order to re-use it
	for (i=0; i<cellCount; i++){
		my_neighbour_count[i] = 0;
	}

	rewind(input_file);
	//fill the buffers while parsing the file again
	for (line_counter=0;line_counter<IO_NETWORK_SIZE;line_counter++) {
		for (i=0; i<IO_NETWORK_SIZE; i++) {
	...
	//parse the file again in order to handle the sending part of the code
	for (line_counter=0;line_counter<IO_NETWORK_SIZE;line_counter++) {
		temporary = list_head;
		for (i=0; i<IO_NETWORK_SIZE; i++) {
			fscanf(input_file, "%lf ", &cond_value);
			if (cond_value==0)						//this connection is considered not existing if conductance = 0
				...
				if ((line_counter>=my_cell_lower_bound)&&(line_counter<my_cell_upper_bound)) {		//these are this core's cells' ROWS (outgoing)
					...
					if (core_to_send==core_id)							//Obviously I do not send to my own core
						...
						if ((k>0)&&(history[core_to_send][k-1]==my_sending_cell))//I have already marked this cell to send core_to_send
							;
						else {
							history[core_to_send] = allocate_space_int(history[core_to_send], temporary->total_cells_to_send);
							history[core_to_send][k] = my_sending_cell;
	...
	//allocate the necessary data in memory for each core we send data to
	temporary = list_head;
	while (temporary != NULL){
		temporary->cells_to_send = (int*) calloc(temporary->total_cells_to_send, sizeof(int));
		temporary->voltages_to_send = (mod_prec*) calloc(temporary->total_cells_to_send, sizeof(mod_prec));
		temporary = temporary->next;
	}

	//fill the buffers in the sending list with the cell ids which are kept in the history of each core
	temporary = list_head;
	while (temporary != NULL){
		for (i=0; i<temporary->total_cells_to_send; i++)
			temporary->cells_to_send[i] = history[temporary->target_core][i];
		temporary = temporary->next;
	}
